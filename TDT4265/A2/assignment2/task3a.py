...
class SoftmaxModel:
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO implement this function (Task 3a)
        # Sum the images X and the weights to get a, use this in the sigmoid function
        z = X@self.w
        y = np.exp(z)/np.sum(np.exp(z), keepdims=True, axis=1)
        return y
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        gradients = X.T@(outputs-targets) + self.l2_reg_lambda*self.w

        self.grad = gradients/len(outputs)
