        ...
        for epoch in range(num_epochs):
            ...
            for X_batch, Y_batch in iter(train_loader):
                ...
                if global_step % num_steps_per_val == 0:
                    ...
                    validation_loss = list(val_history["loss"].values())
                    
                    # print(validation_loss[counter], "vs", validation_loss[counter-1])
                    if min(validation_loss[-10:]) > min(validation_loss): # this did work :)
                    # if validation_loss[counter] < validation_loss[counter-1]: this did not work :(
                        # print("We stopping on epoch:", epoch, "global step:", global_step)
                        early_stop = False # Turned off on for task 4, set this to True to trun back on
                    # counter += 1
                global_step += 1
