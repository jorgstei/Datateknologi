...
def pre_process_images(X: np.ndarray):
    ...
    X = X.astype(float)
    normalized_arr = np.zeros((X.shape[0], X.shape[1] + 1), dtype=float)
    ...
    for i in range(len(X)):
        for j in range(len(X[0])):
            # Normalize
            normalized_arr[i][j] = 2*(X[i][j]/255) - 1

        # Bias trick
        normalized_arr[i][X.shape[1]] = 1.0
...
def cross_entropy_loss(targets: np.ndarray, outputs: np.ndarray) -> float:
    ...
    total_cost = np.sum(-(targets * np.log(outputs) + (1 - targets) * np.log(1-outputs)), axis=0)
...
class BinaryModel:
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        for i in range(len(X)):
            output[i] = 1/(1+ math.exp(-np.dot(X[i], self.w)))
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        for i in range(len(outputs)):
            gradients += -(targets[i]-outputs[i]) * X[i]

        self.grad = (gradients/len(outputs)).reshape((785, 1))

        #print("Backward:", self.grad)
...
if __name__ == "__main__":
    ...
    print("Done with forward pass test")
